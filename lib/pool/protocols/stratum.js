...
const StratumClient = function (options) {
    ...
    this.sendDifficulty = (difficulty) => {
        if (!self.authorized) {
            //console.log(`Client not authorized: ${self.getLabel()}`);
            return false;
        }
        //console.log(`difficulty ${difficulty} this.difficulty ${this.difficulty} self.difficulty ${self.difficulty}`);
        //difficulty = self.config.ports[options.socket.localPort].diff;
        ...
        self.difficulty = difficulty;
        //console.log(`New difficulty set for ${self.getLabel()}: ${difficulty}.`);
        ...
        //console.log(`Sending target ${target} to miner ${self.getLabel()}`);
        sendJson(self.socket, {
            id: null,
