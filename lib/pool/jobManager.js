...
class JobManager extends events.EventEmitter {
    ...
    async processTemplate(rpcData) {
        ...
        if (!newBlock && newDiff) {
            if (this.currentJob) {
                const targeta = BigInt('0x' + this.currentJob.rpcData.target);
                const targetb = BigInt('0x' + rpcData.target);
                const diffa = parseFloat((algos.komodo.diff1 / Number(targeta)).toFixed(9));
                const diffb = parseFloat((algos.komodo.diff1 / Number(targetb)).toFixed(9));
        ...
        this.difficulty = parseFloat((algos.komodo.diff1 / Number(this.target)).toFixed(9)); // Use komodo.diff1 for internal calculations
    ...
    processShare(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, soln) {
        ...
        if (extraNonce1.length >= 64) {
            return shareError([20, 'incorrect size of extraNonce1']);
        }
        if (extraNonce2.length !== 64 - extraNonce1.length) {
        ...
        const headerBigInt = util.bufferToBigInt(headerHash);

        let blockHashInvalid;
        let blockHash;
        let blockHex;

        const shareDiff = (algos.komodo.diff1 / Number(headerBigInt));
        const blockDiffAdjusted = job.difficulty;

        // Check if the share is a valid block candidate
        if (headerBigInt <= job.target) {
