...
const BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, extraNoncePlaceholder, reward, poolAddress, coin, pubkey) {
    ...
    this.difficulty = parseFloat((algos.komodo.diff1 / this.target.toNumber()).toFixed(9)); // Calculate difficulty
...
BlockTemplate.prototype.generateMerkleRoot = function() {
    this.prevHashReversed = util.reverseHex(this.rpcData.previousblockhash); // Reverse the previous block hash
    this.hashReserved = util.reverseHex(this.rpcData.finalsaplingroothash); // Reverse the final sapling root hash
    this.merkleRoot = merkle.getRoot(this.rpcData, this.genTxHash); // Generate the merkle root
    this.merkleRootReversed = util.reverseHex(this.merkleRoot); // Reverse the merkle root
};
