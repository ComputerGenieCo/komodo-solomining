...
const peer = require('@protocols/peer.js');
const { interface: DaemonInterface } = require('@protocols/daemon.js'); // Renamed to DaemonInterface
const stratum = require('@protocols/stratum.js');
...
class Pool extends events.EventEmitter {
    ...
    constructor(options, authorizeFn) {
        ...
        this.blockUtils = require('@blockchain/blockUtils.js')(this.daemon, emitErrorLog, emitWarningLog, emitLog);
        const { GetBlockTemplate, DetectCoinData } = require('@blockchain/blockUtils.js')(this.daemon, emitErrorLog, emitWarningLog, emitLog);
    ...
    setupPeer() {
        ...
        this.peer.on('connected', () => {
            ...
            emitErrorLog('p2p connection failed - likely incorrect config of pool or daemon');
        }).on('disconnected', () => {
     ...
     */
    async relinquishMiners(filterFn, resultCback) {
        ...
        const clientsToRelinquish = await Promise.all(
            stratumClients.map(async cObj => {
                const shouldRelinquish = await filterFn(cObj);
                return shouldRelinquish ? cObj : null;
            })
        ).then(results => results.filter(cObj => cObj !== null));
